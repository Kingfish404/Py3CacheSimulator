...
class Cache:
    def __init__(self, name, word_size, block_size, n_blocks, associativity, hit_time, write_time, write_back, logger, next_level=None):
        ...
        if next_level:
            for i in range(self.n_sets):
                ...
                if index == '':
                    index = '0'
                # Create a dictionary of blocks for each set
                self.data[index] = {}
                ...
                if len(in_cache) < self.associativity:
                    ...
                    if self.write_back:
                        if self.data[index][oldest_tag].is_dirty():
                            self.logger.info(
                                '\tWriting back block ' + address + ' to ' + self.next_level.name)
            ...
            if tag in in_cache:
                ...
                if self.write_back:
                    ...
                    self.logger.info('\tWriting through block ' +
                                     address + ' to ' + self.next_level.name)
    ...
    def parse_address(self, address):
        ...
        index = binary_address[-(self.block_offset_size +
                                 self.index_size):-self.block_offset_size]
        if index == '':
            index = '0'
